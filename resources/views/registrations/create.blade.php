...
@section('content')
<div class="container">
    <h3>Registrations Management - New Registration</h3>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <select @if(request()->get('id') !== null) {{ 'readonly'}} @endif id="event_id" type="text" class="form-control @error('event_id') is-invalid @enderror" name="event_id" value="{{ old('event_id') }}" autocomplete="event_id" autofocus>
                                <option value="">Select</option>
                                <?php $events = App\Models\Event::where('from_date', '>=', date('Y-m-d', strtotime(today())))
                                    ->orderBy('name', 'asc')->get();?>
                                    @foreach($events as $event)
                                        <option value="{{ $event->id }}" @if(old('event_id') == $event->id || request()->get('id') == $event->id) {{ 'selected' }} @endif>{{ $event->name }} ({{ date('M d', strtotime($event->from_date)) }} - {{ date('M d, Y', strtotime($event->to_date)) }})</option>
                                    @endforeach
                            </select>
                            @error('event_id')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <input id="office_name" type="text" class="form-control @error('office_name') is-invalid @enderror" name="office_name" value="{{ old('office_name') }}" autocomplete="office_name">
                            <input id="status" type="hidden" class="form-control @error('status') is-invalid @enderror" name="status" value="{{ old('status') ?? 1 }}" autocomplete="status">
